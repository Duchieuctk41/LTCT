...
void TimKiemNV_Nam(NhanVien a[max], int n, int nam)
{
  int vitri[max],
      m = 0;
  for (int i = 0; i < n; i++)
  ...
  if (m == 0)
  {
    cout << "\nKhong co nhan vien sinh nam " << nam << " trong danh sach.";
  }
  else
  {
    XuatTieuDe();
...
void SapXepNV_Nam_MS(NhanVien a[max], int n)
{
  for (int i = 0; i < n - 1; i++)
    for (int j = i + 1; j < n; j++)
...
void TimKiemNV_Ten(NhanVien a[max], int n, char ten[8])
{
  int vitri[max],
      m = 0;
  for (int i = 0; i < n; i++)
...
void SapXepNV_Luong_MS(NhanVien a[max], int n)
{
  for (int i = 0; i < n - 1; i++)
    for (int j = i + 1; j < n; j++)
